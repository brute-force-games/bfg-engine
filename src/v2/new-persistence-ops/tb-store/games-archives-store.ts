...
import { GameRoomPersistSchema } from '../models/tinybase/game-room-persist';
...
// Helper type to ensure we have a gameStep property
// type HasGameStep = { gameStep: GameBoardEventForDb };
...
export const saveNewHostedGame = async (
  ...
  try {
    ...
    // Convert GameTableEventForGameStep to GameBoardEventForDb
    // Extract the gameStep from the nested structure
    const gameMetadata = getGameMetadata(gameRoom.gameTitle);
    const GameTableEventSchema = gameMetadata.schemas.gameTableEventSchema;
    // type GameRoomEvent = z.infer<typeof GameRoomEventSchema>;
    
    // Extract gameStep from the event - TypeScript knows this exists from the function signature
    // const gameStep = initialGameTableEventWithTransition.gameStep;
    
    // Validate the gameStep matches the expected GameBoardEventForDb structure
