...
import { updateHostedGameWithNewNextBoardState, useLatestHostedGameSnapshot, useLatestPerfectInformationGameJournal } from "../../../../tb-store/games-archives-store";
...
import type { GameRoomModeHostOnlyAccess } from "../p2p-game-types";
import { useGameInstanceUserDetails } from "../use-bfg-game-instance";
import { GTAL_PLAYER } from "../../../../game-metadata/metadata-types";
...
export const adaptToHostedGameRoomNoP2p = (
  hostedRoom: GameRoomModeHostOnlyAccess,
): IBfgGameTableForHost | null => {

  const hostedGameRoomContext = hostedRoom.hostedGameRoom;
  // const p2pGameRoom = hostedRoom;

  const gameInstanceUserDetails = useGameInstanceUserDetails(hostedRoom.gameInstanceId, 'host');
  ...
  const hostedGameSnapshot = useLatestHostedGameSnapshot(hostedRoom.gameInstanceId);
  if (!hostedGameSnapshot) {
    console.error('❌ Hosted game room snapshot not found');
    return null;
  }

  const perfectInformationGameJournal = useLatestPerfectInformationGameJournal(hostedRoom.gameInstanceId);
  ...
  useEffect(() => {
    ...
    if (hostedGameTableEvents.length > 0) {
      ...
      // console.log('🎮 Latest event stepIndex:', latestEvent.stepIndex, 'source:', latestEvent.event.source);
  ...
  const latestGameState = gameMetadata.schemas.hostGameStateSchema.parse(latestTableEvent.nextBoardState);
  ...
  const emptyP2pDetails: IP2pDetails = {
    peerIds: [],
  ...
  const p2pDetails: IP2pDetails = emptyP2pDetails;
  ...
  const onPlayerAction = useCallback(async (playerAction: BfgGameActionByPlayer) => {
    ...
    const playerActionEvent: GameTableEventForDb = {
      stepIndex: gameStep.stepIndex,
      createdAt: gameStep.createdAt,
    ...
    updateHostedGameWithNewNextBoardState(hostedRoom.gameInstanceId, nextStepIndex, playerActionEvent);
    ...
    //   updateHostedGame(hostedGameState.id, updatedGameTable, updatedGameEvent, updatedGameEventChange, updatedNextGameState);
    // }
  }, [gameMetadata, gameRegistry, hostedGame, hostedGameSnapshot, gameInstanceUserDetails.myHostProfile, latestGameState, latestStepIndex, hostedRoom.gameInstanceId]);
  ...
  const onHostAction = useCallback(async (hostAction: BfgGameActionByHost) => {
    ...
    updateHostedGameWithNewNextBoardState(hostedRoom.gameInstanceId, nextStepIndex, hostActionEvent);
  ...
  const allPlayerProfiles = hostedGame.players
    .map(player => player.playerProfile)
    .reduce((acc, playerProfile) => {
      acc.set(playerProfile.id, playerProfile);
      return acc;
    }, new Map<PlayerProfileId, PublicPlayerProfile>());
  ...
  const playerGameDetails: IPlayerBfgGameDetails | null = 
    ...
    gameInstanceUserDetails.allowedLevels.includes(GTAL_PLAYER) ? {
      gameMetadata,
  ...
  const retVal: IBfgGameTableForHost = {
    gameInstanceId: hostedRoom.gameInstanceId,
    gameMetadata,
